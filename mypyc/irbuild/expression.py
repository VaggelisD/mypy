...
def _codepoint_kind(builder: IRBuilder, expr: Expression, expr_type: RType) -> str | None:
    """Cheap check (no IR emission): classify expr as a codepoint candidate.
    Returns "char" for char-typed values, "index" for ``s[i]`` on a str with
    int-like index, or None. Call _emit_codepoint_value once we commit.
    """
...
def _emit_codepoint_value(
    ...
    """Emit the codepoint read. `kind` must come from a preceding
    _codepoint_kind call — caller commits to the fast path here.
    """
    if kind == "char":
...
def try_specialize_codepoint_compare(
    ...
    """Specialize == / != comparisons where at least one side is a codepoint
    (char-typed value or ``s[i]`` on a str) and the other is either another
    codepoint or a 0/1-char str literal. Rewrites to an integer compare of the
    codepoint, avoiding 1-char PyObject allocation and PyUnicode_Compare.
    """
    if op not in ("==", "!="):
        return None
    lhs_type = builder.node_type(lhs)
    rhs_type = builder.node_type(rhs)
    lhs_kind = _codepoint_kind(builder, lhs, lhs_type)
    rhs_kind = _codepoint_kind(builder, rhs, rhs_type)
    ...
    # One side codepoint, other side must be a 0/1-char str literal.
    # Empty string is encoded as -1 (the char empty sentinel).
    ...
    if not isinstance(folded, str) or len(folded) > 1:
        return None  # No IR emitted — bail before committing.
