...
// Shared helper for ASCII + delegated Unicode case conversion. Returns -113
// (the char error sentinel) on OOM / CPython error, with the exception set.
...
// .upper() / .lower(): ASCII fast path; non-ASCII delegates to the str
// method. If the result is multi-codepoint (e.g. ß -> SS), the original
// codepoint is returned since char holds one codepoint. Propagates OOM /
// errors from CPython by returning -113 with the exception set.
